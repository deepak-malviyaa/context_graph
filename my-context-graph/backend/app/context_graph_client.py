...
async def connect_neo4j() -> None:
    ...
    _driver = AsyncGraphDatabase.driver(
        ...
        # Aura / cloud NATs silently drop idle bolt sockets. Recycle pooled
        # connections aggressively and check liveness before reuse so the
        # next query doesn't hit a half-open TCP socket.
        max_connection_lifetime=300,           # seconds — Aura recommends <=3600
        liveness_check_timeout=30,             # ping idle connections >30s old
...
async def execute_cypher(
    ...
    *,
    ...
    # Retry once on a defunct/expired pooled connection (Aura drops idle sockets).
    for attempt in range(2):
        try:
            async with driver.session(database=settings.neo4j_database) as session:
                ...
                return records
        except (SessionExpired, ServiceUnavailable, ConnectionResetError) as e:
            if attempt == 0:
                logger.warning("Neo4j connection dropped, retrying once: %s", e)
                continue
            raise
    return []
