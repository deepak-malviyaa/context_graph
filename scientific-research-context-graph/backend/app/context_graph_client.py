...
async def connect_neo4j() -> None:
    ...
    # Aura's load balancer drops idle TCP connections after ~30 min, which then
    # surface as ConnectionResetError(10054, 'forcibly closed by the remote host').
    # We recycle pooled connections well before that and enable TCP keepalives so
    # the OS detects dead sockets quickly.
    _driver = AsyncGraphDatabase.driver(
        settings.neo4j_uri,
        auth=(settings.neo4j_username, settings.neo4j_password),
        max_connection_lifetime=20 * 60,       # 20 min < Aura's idle cap
...
async def execute_cypher(
    ...
    *,
    ...
    Uses a managed transaction (``session.execute_read``) so the Neo4j driver
    transparently retries the query on transient connection failures —
    ``ConnectionResetError`` from Aura's idle-timeout / load-balancer eviction,
    ``ServiceUnavailable``, ``SessionExpired``, ``TransientError`` — without
    surfacing them to the caller. An outer retry loop with exponential backoff
    handles the rare case where the driver gives up after its own attempts.
    """
    from neo4j.exceptions import (
        ServiceUnavailable, SessionExpired, TransientError, DriverError,
    )
    ...
    async def _run(tx) -> list[dict]:
        result = await tx.run(query, params, timeout=timeout)
        out: list[dict] = []
        async for record in result:
            out.append({k: _serialize(v) for k, v in record.items()})
        return out
    ...
    for attempt in range(4):
        try:
            async with driver.session(database=settings.neo4j_database) as session:
                # Managed transactions auto-retry on transient failures inside
                # the driver itself; this is the official remedy for the
                # "forcibly closed by the remote host" log lines from Aura.
                # We use execute_write because execute_cypher is also used for
                # writes (MERGE/SET in the seed script and /cypher route);
                # write txns happily run read queries too.
                records = await session.execute_write(_run)
                if collect:
                    _collector.collect(records)
                    if tool_name:
                        _collector.collect_tool_call(
                            tool_name, params, str(records[:2])
                        )
                return records
        except (ServiceUnavailable, SessionExpired, TransientError,
                DriverError, OSError) as e:
            last_exc = e
            logger.warning(
                "Neo4j transient error on attempt %d/4 (%s: %s); retrying in %.1fs",
                attempt + 1, e.__class__.__name__, e, backoff,
            )
