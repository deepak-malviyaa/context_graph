...
export function ContextGraphView({ externalGraphData, onAskAbout }: ContextGraphViewProps) {
  ...
  return (
    <Box h="100%" position="relative">
      ...
      >
        <Box flexShrink={0}>
          <Heading size="sm" className="panel-heading">Knowledge Graph</Heading>
          <Text fontSize="xs" className="subtitle">
            {isSchemaView
              ? "Schema view — double-click a label to explore"
              : "Scientific Research entity relationships"}
          </Text>
        </Box>
        <Box className="kpi-strip" display={{ base: "none", md: "flex" }}>
          <Box className="kpi-card kpi-nodes">
            <Box className="kpi-label">Nodes</Box>
            <Box className="kpi-value">{graphData?.nodes.length ?? 0}</Box>
          </Box>
          <Box className="kpi-card kpi-edges">
            <Box className="kpi-label">Edges</Box>
            <Box className="kpi-value">{graphData?.relationships.length ?? 0}</Box>
          </Box>
          <Box className="kpi-card kpi-types">
            <Box className="kpi-label">Labels</Box>
            <Box className="kpi-value">{new Set((graphData?.nodes ?? []).flatMap(n => n.labels)).size}</Box>
          </Box>
          <Box className="kpi-card kpi-mode">
            <Box className="kpi-label">Mode</Box>
            <Box className="kpi-value" style={{ fontSize: 13 }}>{isSchemaView ? "Schema" : "Data"}</Box>
          </Box>
        </Box>
        {!isSchemaView && (
          <IconButton
      ...
      <Flex
        ...
        maxH="180px"
        overflowY="auto"
        css={{ "&::-webkit-scrollbar": { width: "4px" }, "&::-webkit-scrollbar-thumb": { background: "linear-gradient(180deg, #6366f1, #06b6d4)", borderRadius: "4px" } }}
      >
            ...
            <VStack align="stretch" gap={2}>
              ...
              <VStack align="stretch" gap={1}>
                {Object.entries((selectedElement.data as GraphNode).properties)
                  ...
                  .map(([key, value]) => (
                    <Box key={key} className="prop-row" fontSize="xs">
      ...
      {(loading || isExpanding) && (
        <Box
          ...
          zIndex={20}
        >
          <Box className="graph-loading-badge">
