...
test.describe("Scientific Research Context Graph", () => {
  ...
  test("page loads with header and chat panel", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Scientific Research/i })).toBeVisible();
  ...
  test("backend health check returns ok or degraded", async ({ request }) => {
    ...
    expect(body.domain).toBe("scientific-research");
  });
  ...
  test("demo prompt: Literature Review — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("Find the most cited papers in computational biology from the last 3 years");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("Find the most cited papers in computational biology from the last 3 years").first()).toBeVisible();
  ...
  test("demo prompt: Research Management — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("What grants are currently active and when do they expire?");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("What grants are currently active and when do they expire?").first()).toBeVisible();
  ...
  test("demo prompt: Collaboration & Impact — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("Find researchers working on similar topics to our lab\u0027s specialization");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("Find researchers working on similar topics to our lab\u0027s specialization").first()).toBeVisible();
  ...
  test("tool calls show timeline with status indicators", async ({ page }) => {
    ...
    const toolBadge = page.locator("[data-scope='badge']").filter({ hasText: /execute_cypher|get_schema|search_researcher/ });
  ...
  test("API: Literature Review prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Find the most cited papers in computational biology from the last 3 years" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Literature Review prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What papers cite the landmark study on CRISPR gene editing by Doudna?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Literature Review prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me the citation network around deep learning in drug discovery" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Research Management prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What grants are currently active and when do they expire?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Research Management prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me all experiments that used the Genomics Benchmark dataset" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Research Management prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Which researchers at our institution have published in Nature?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Collaboration & Impact prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Find researchers working on similar topics to our lab\u0027s specialization" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Collaboration & Impact prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What is the h-index distribution across our department?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Collaboration & Impact prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Which of our papers have been replicated by independent groups?" },
    });
    expect(res.ok()).toBeTruthy();
